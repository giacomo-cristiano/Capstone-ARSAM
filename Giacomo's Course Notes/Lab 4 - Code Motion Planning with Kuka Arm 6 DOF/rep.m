function tau = rep(q, myrobot, obs)
    ...
    for i = 1:6
        ...
        if obs.type == 'sph' % b = center + R * unit vector in dir of O
            center_dist = O_init(:,i) - obs.c;
            b = obs.c + obs.R * (center_dist)/norm(center_dist);

        elseif obs.type == 'cyl' % only have c(x,y); append on O(z) for b
            center_dist = O_init(1:2,i) - obs.c;
            if O_init(3,i) > obs.h % O(z) > h; above cylinder
                if norm(center_dist) < obs.R % directly above cylinder
                    % treat like plane
                    b = [O_init(1:2,i); obs.h];
                else % above cylinder but not directly; take top rim as b
                    b = [obs.c + obs.R * (center_dist)/norm(center_dist); obs.h];
                end
            else % O(z) < h; in line with cylinder; normal case
